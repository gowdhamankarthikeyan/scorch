            ...
            {  
                switch (selectionMethod)
                {
                        ...
                        else
                        {
                            ...
                            foreach (IResultObject comp in obj)
                            {
                                ...
                                foreach (String siteCode in DP.AgentSite.Split(','))
                                {
                                    try
                                    {
                                        CM2012Interop.refreshSCCMPackageAtDistributionPoint(connection, pkgID, siteCode, criteria, NALPathQuery);
                                        success = true;
                                        break;
                                    }
                                    catch { }
                    ...
                    case "Distribution Point Group":
                        ...
                        {
                            CM2012Interop.refreshSCCMPackageAtDistributionPointGroup(connection, pkgID, criteria);
                        }
                        break;
                    case "All Distribution Points":
                        CM2012Interop.refreshSCCMPackageAtAllDistributionPoints(connection, pkgID);
                        break;
                }
