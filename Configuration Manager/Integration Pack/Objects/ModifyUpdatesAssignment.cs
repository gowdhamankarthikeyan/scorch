...
namespace SCCMExtension
{
    ...
    public class ModifyUpdatesAssignment : IActivity
    {
        ...
        public void Design(IActivityDesigner designer)
        {
            ...
            using(connection)
            { 
                String[] propertyNameChoices = CMInterop.getSCCMObjectPropertyNames(connection, "SMS_UpdatesAssignment");
        ...
        public void Execute(IActivityRequest request, IActivityResponse response)
        {
            ...
            using (connection)
            {
                ...
                foreach (String propertyName in propertyNameChoices)
                {
                    ...
                    {
                        CMInterop.modifySCCMUpdatesAssignment(connection, objID, request.Inputs[(propertyName + " : Property Type")].AsString(), propertyName, request.Inputs[(propertyName + " : Property Value")].AsString());
                    }
                }

                IResultObject col = CMInterop.getSCCMUpdatesAssignment(connection, "AssignmentID LIKE '" + objID + "'");
