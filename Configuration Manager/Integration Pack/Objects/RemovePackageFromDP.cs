            ...
            {  
                switch (selectionMethod)
                {
                        ...
                        else
                        {
                            ...
                            foreach (IResultObject comp in obj)
                            {
                                ...
                                foreach (String siteCode in DP.AgentSite.Split(','))
                                {
                                    try
                                    {
                                        CMInterop.removeSCCMPackageFromDistributionPoint(connection, pkgID, siteCode, criteria, NALPathQuery);
                                        success = true;
                                        break;
                                    }
                                    catch { }
                    ...
                    case "Distribution Point Group":
                        ...
                        {
                            CMInterop.removeSCCMPackageFromDistributionPointGroup(connection, pkgID, criteria);
                        }
                        break;
                    case "All Distribution Points":
                        CMInterop.removeSCCMPackageFromAllDistributionPoints(connection, pkgID);
                        break;
                }
