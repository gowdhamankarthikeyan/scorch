...
{
    [Activity("Modify SCCM Software Updates Package")]
    public class ModifySoftwareUpdatesPackage : IActivity
    {
        ...
        public void Design(IActivityDesigner designer)
        {
            ...
            using(connection)
            {  
                String[] propertyNameChoices = CMInterop.getSCCMObjectPropertyNames(connection, "SMS_SoftwareUpdatesPackage");
        ...
        public void Execute(IActivityRequest request, IActivityResponse response)
        {
            ...
            try
            {
                ...
                foreach (String propertyName in propertyNameChoices)
                {
                    ...
                    {
                        CMInterop.modifySCCMSoftwareUpdatesPackage(connection, objID, request.Inputs[(propertyName + " : Property Type")].AsString(), propertyName, request.Inputs[(propertyName + " : Property Value")].AsString());
                    }
                }

                IResultObject col = CMInterop.getSCCMSoftwareUpdatesPackage(connection, "PackageID LIKE '" + objID + "'");
                if (col != null)
                {
